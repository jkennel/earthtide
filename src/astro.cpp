...
library(fst)
library(bench)
wl <- read_fst('/media/kennel/Data/phd/posters/agu_2018/rd130_077623_20161115_1344.fst', as.data.table = TRUE)

#wl <- wl[seq(1, nrow(wl)/2, 60)]
wl <- wl[1:(86400*1)]

# wl <- wl[datetime %between% as.POSIXct(c('2016-08-13', '2016-08-14'), tz = 'UTC')]

tms <- wl$datetime 

freq_range <- na.omit(eterna_wavegroups[eterna_wavegroups$time == '1 month',])
freq_range <- freq_range[freq_range$start > 0.7,][, c('start', 'end')]


astro_update = 3600L
update_coef <- pi / 180.0 * 1 / 3600.0
aa <- earthtide::Earthtide$new(tms, latitude = 34.23,
                               longitude = -118.67,
                               elevation = 521,
                               azimuth = 0,
                               gravity = 0,
                               ...
                               catalog = 'hw95s')
aa$station$dgk <- aa$station$dgz
aa$pk[] <- 180 

aa$delta[1:12] <- aa$love_params$dglat
aa$deltar <- aa$love_params$dgr

tmp <- bench::mark(
  
  # tmp0 <- et_predict(aa$astro$astro,
  #                               aa$astro$astro_der,
  #                               aa$catalog$k,
  #                               aa$pk,
  #                               aa$delta,
  ...
  #                               aa$love_params$domr,
  #                               aa$catalog$id,
  #                               aa$datetime$astro_update),
  tmp0 <- et_calculate(aa$astro$astro,
                     aa$astro$astro_der,
                     aa$catalog$k,
                     aa$pk,
                     aa$delta,
                     ...
                     update_coef,
                     TRUE),
  tmp1 <- et_calculate(aa$astro$astro,
                     aa$astro$astro_der,
                     aa$catalog$k,
                     aa$pk,
                     aa$delta,
                     ...
                     update_coef,
                     FALSE),
  # tmp2 <- et_analyze(aa$astro$astro,
  #                    aa$astro$astro_der,
  #                    aa$catalog$k,
  #                    aa$pk,
  #                    aa$delta,
...
plot(tmp0, type='l', col='red')

plot(ttttt, type='l')


points(tmp1[ind], type='l', col='red')
points(tmp2[ind], type='l', col='blue')
points(tmp4[ind], type='l', col='green')

tmp
max(abs(tmp2-tmp1))
all.equal(tmp2, tmp1, tolerance = 1e-11)
